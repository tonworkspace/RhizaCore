...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'users' AND column_name = 'squad_mining_rate') THEN
        ALTER TABLE users ADD COLUMN squad_mining_rate DECIMAL(10, 2) DEFAULT 25.0;
    END IF;
END $$;

-- Function to calculate squad size (active referrals)
CREATE OR REPLACE FUNCTION get_user_squad_size(user_id_param UUID)
RETURNS INTEGER AS $$
BEGIN
    RETURN (
...
CREATE OR REPLACE FUNCTION can_claim_squad_rewards(user_id_param UUID)
RETURNS BOOLEAN AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION calculate_squad_reward(user_id_param UUID)
...
CREATE OR REPLACE FUNCTION get_squad_mining_stats(user_id_param UUID)
RETURNS JSON AS $$
DECLARE
    squad_size INTEGER;
...
GRANT EXECUTE ON FUNCTION get_user_squad_size(UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION can_claim_squad_rewards(UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION calculate_squad_reward(UUID) TO authenticated;
GRANT EXECUTE ON FUNCTION claim_squad_mining_rewards(UUID, TEXT) TO authenticated;
GRANT EXECUTE ON FUNCTION get_squad_mining_stats(UUID) TO authenticated;
...
COMMENT ON FUNCTION get_user_squad_size(UUID) IS 'Returns the number of active referrals (squad members) for a user';
COMMENT ON FUNCTION can_claim_squad_rewards(UUID) IS 'Checks if user can claim squad rewards (8-hour cooldown)';
COMMENT ON FUNCTION calculate_squad_reward(UUID) IS 'Calculates potential squad mining reward based on squad size';
COMMENT ON FUNCTION claim_squad_mining_rewards(UUID, TEXT) IS 'Claims squad mining rewards and updates user balance';
COMMENT ON FUNCTION get_squad_mining_stats(UUID) IS 'Returns comprehensive squad mining statistics for a user';
-- Create wallet activation system
...
-- Add RLS policies for wallet_activations table
ALTER TABLE wallet_activations ENABLE ROW LEVEL SECURITY;

-- Users can view their own activation records
CREATE POLICY "Users can view their own activations" ON wallet_activations
    FOR SELECT USING (true); -- Disable RLS for now, same as user_transfers

-- Users can insert their own activation records
CREATE POLICY "Users can create their own activations" ON wallet_activations
    FOR INSERT WITH CHECK (true); -- Disable RLS for now

-- Disable RLS for compatibility with current system
...
-- Test data insertion (only if user ID 1 exists)
-- This will be skipped if no users exist yet
DO $$
BEGIN
    -- Only insert test data if user ID 1 exists
    IF EXISTS (SELECT 1 FROM users WHERE id = 1) THEN
        ...
        VALUES (1, 2.5, 15.00, 6.00, 150.0, 'test_hash_123', 'test_sender_address', 'test_receiver_address', 'confirmed', NOW())
        ON CONFLICT DO NOTHING;
        
        RAISE NOTICE 'Test activation record created for user ID 1';
    ELSE
        RAISE NOTICE 'No test data inserted - user ID 1 does not exist';
    END IF;
END $$;
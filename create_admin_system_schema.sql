...
-- Complete admin user management and authentication system
-- ===================================================================
...
CREATE OR REPLACE FUNCTION add_admin_user(
    ...
    p_added_by INTEGER DEFAULT NULL
)
RETURNS JSON AS $$
DECLARE
...
CREATE OR REPLACE FUNCTION remove_admin_user(
    p_user_id INTEGER,
    p_removed_by INTEGER DEFAULT NULL
)
RETURNS JSON AS $$
BEGIN
...
-- 1. Initialize admin system with first admin
...
SELECT update_admin_permissions(123, ARRAY['user_management', 'activation', 'reports'], 1);

-- 6. Remove admin privileges
...
GRANT EXECUTE ON FUNCTION add_admin_user(INTEGER, VARCHAR(20), TEXT[], INTEGER) TO authenticated;
GRANT EXECUTE ON FUNCTION remove_admin_user(INTEGER, INTEGER) TO authenticated;
GRANT EXECUTE ON FUNCTION update_admin_permissions(INTEGER, TEXT[], INTEGER) TO authenticated;
...
-- Uncomment to create sample admin user (replace with actual user ID)
-- SELECT initialize_admin_system(1, 'super');